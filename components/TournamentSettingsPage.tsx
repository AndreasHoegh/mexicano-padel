...
export default function TournamentSettingsPage({
  ...
  const addCourt = () => {
    if (courts.length < getMaxCourts(playerCount)) {
      setCourts((prev) => [
        ...prev,
        { id: nextIdRef.current, name: `Court ${nextIdRef.current}` },
      ]);
      nextIdRef.current += 1;
    }
  };
  ...
  const removePlayer = (index: number) => {
    ...
    for (let i = index; i < playerCount - 1; i++) {
      ...
      setValue(
        `playerName${i}`,
        nextValue || `${mode === "team" ? "Team" : "Player"}${i + 1}`
      );
    }
  ...
  const addPlayer = () => {
    setPlayerCount(playerCount + (mode === "team" ? 2 : 1));
  };

  const handleSettingsSubmit = (data: FieldValues) => {
  ...
  return (
    <div className="flex flex-col items-center mt-4">
      <form onSubmit={handleSubmit(handleSettingsSubmit)} className="space-y-8">
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              ...
              onValueChange={(value: "individual" | "team") => {
                setMode(value);
                setPlayerCount(8); // Reset to default when changing mode
              }}
            >
              {[
                {
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-center">
              {mode === "team" ? "Team Names" : "Player Names"}
            </h2>
            {mode === "team" ? (
          ...
          {/* Final Round Pairing (Individual mode only) */}
          {mode === "individual" && (
            <div className="space-y-3">
              <h2 className="text-lg font-semibold text-center">
                Final Round Pairing
